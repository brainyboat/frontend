<script>
  import { fade } from "svelte/transition";
</script>

<div
  class="vh-100 d-flex flex-column justify-content-center align-items-center bg"
  in:fade|local
>
  <div class="shadow p-sm-5 p-4 bg-light m-3" style="max-width: 42rem">
    <main>
      <h1 class="border-bottom pb-3 mb-4">
        <svg
          style="width: calc(1.375rem + 1.25vw)"
          class="me-1 pb-2 h-100 my-auto"
          xmlns="http://www.w3.org/2000/svg"
          height="40"
          viewBox="0 -960 960 960"
          width="40"
        >
          <path
            d="m269-269 292-129 129-292-292 129-129 292Zm211-171q-17 0-28.5-11.5T440-480q0-17 11.5-28.5T480-520q17 0 28.5 11.5T520-480q0 17-11.5 28.5T480-440Zm.138 385Q392-55 314.513-88.084q-77.488-33.083-135.417-91.012T88.084-314.375Q55-391.724 55-479.862 55-569 88.084-646.487q33.083-77.488 90.855-134.969 57.772-57.482 135.195-91.013Q391.557-906 479.779-906q89.221 0 166.827 33.454 77.605 33.453 135.012 90.802 57.407 57.349 90.895 134.877Q906-569.34 906-480q0 88.276-33.531 165.747-33.531 77.471-91.013 135.278-57.481 57.808-134.831 90.891Q569.276-55 480.138-55ZM480-149q138 0 234.5-96.372T811-480q0-138-96.5-234.5t-235-96.5q-137.5 0-234 96.5t-96.5 235q0 137.5 96.372 234T480-149Zm0-331Z"
          />
        </svg>
        BrainyBoat
      </h1>
      <h3>О нас</h3>
      <p>
        Компания BrainyBoat предоставляет универсальный аппаратно-программный
        комплекс для обеспечения автономного судовожденя.
      </p>
      <h3>Контакты</h3>
      <ul>
        <li>
          E-mail: <a href="mailto:mail@brainyboat.ru">mail@brainyboat.ru</a>
        </li>
        <li>Telegram: <a href="https://t.me/brainyboat">@brainyboat</a></li>
      </ul>

      <a class="btn btn-primary" href="/#/dashboard">
        Перейти в панель управления
      </a>
    </main>
  </div>
</div>

<style>
  .bg {
    background-image: url("../../assets/lbg.jpg");
    background-size: cover;
    background-position: center;
  }
</style>
